...
import { trpc, postFetcher } from '../../util'; // 导入 fetcher 函数
...
async function handleDeleteRoom(roomId: number) {
  try {
    ...
    await trpc.room.delete.useMutation({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ['room.list'], // 指定查询键
        }); // 手动刷新房间列表
...
const sortRoomsByLastTime = (rooms: RoomPreviewInfo[]) => {
  ...
  const renderRoomEntries = () => {
    ...
    const sortedRooms = sortRoomsByLastTime(roomData.rooms);
            ...
            <div className="chat_bg">
              <div className="chat_head">
                ...
                <span className="room_name">
                  {roomData?.rooms.find((room: { roomId: number | null; }) => room.roomId === roomId)?.roomName || ""}
                </span>
              </div>
              <div id="chat" className="chat" ref={chatAreaRef}>
                {messageListData?.messages.map((msg: { sender: string; time: string; content: string; }, index: React.Key | null | undefined) => (
                  <MessageItem 
                  key={index}
                  sender={msg.sender}
                  sendTime={msg.time}
